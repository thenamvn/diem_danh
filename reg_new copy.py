    ...
    def run(self):
        if self.frame_rgb is not None:
            ...
            if face_locations:
                ...
                crop_x2 = min(
                    self.frame_rgb.shape[1], face_center[0] + crop_width // 2)
                crop_y2 = min(
                    self.frame_rgb.shape[0], face_center[1] + crop_height // 2)
                cropped_frame_extended = self.frame_rgb[crop_y1:crop_y2,
                                                        crop_x1:crop_x2]

                self.face_processed_signal.emit(
                    cropped_face, cropped_frame_extended, self.name, self.student_id)
    ...
    def run(self):
        try:
            if self.face_image is not None and self.extended_image is not None:
                ...
                _, face_encoded = cv2.imencode(
                    '.jpg', cv2.cvtColor(self.face_image, cv2.COLOR_RGB2BGR))
                face_data = face_encoded.tobytes()
                face_blob = bucket.blob(
                    f'faces_data/{self.student_id}_face.jpg')
                if face_blob.exists():
                    face_blob.delete()
                self.progress_signal.emit(50, "Uploading face image...")
                face_blob.upload_from_string(
                    face_data, content_type='image/jpeg')
                face_blob.make_public()
                face_image_url = face_blob.public_url + \
                    f"?cachebust={datetime.now().timestamp()}"

                _, extended_encoded = cv2.imencode('.jpg', cv2.cvtColor(
                    self.extended_image, cv2.COLOR_BGR2RGB))  # Ensure RGB format
                extended_data = extended_encoded.tobytes()
                extended_blob = bucket.blob(
                    f'faces_data/{self.student_id}_extended.jpg')
                if extended_blob.exists():
                    extended_blob.delete()
                self.progress_signal.emit(
                    75, "Uploading extended face image...")
                extended_blob.upload_from_string(
                    extended_data, content_type='image/jpeg')
                ...
                self.progress_signal.emit(
                    100, "Upload to Firebase successful!")
                self.upload_complete_signal.emit(
                    True, "Upload to Firebase successful!")
            else:
                self.upload_complete_signal.emit(
                    False, "Face or extended image is missing")
                self.progress_signal.emit(0, "")
        except Exception as e:
            self.upload_complete_signal.emit(
                False, f"Error uploading to Firebase: {e}")
            self.progress_signal.emit(0, "")
...
class FaceRecognitionApp(QMainWindow):
    def __init__(self):
        ...
        self.face_recognition_thread.face_processed_signal.connect(
            self.start_upload)
        self.face_recognition_thread.face_detection_failed_signal.connect(
            self.handle_face_detection_failed)
        self.face_recognition_thread.progress_signal.connect(
            self.update_progress)

        self.firebase_upload_thread = FirebaseUploadThread(self)
        self.firebase_upload_thread.upload_complete_signal.connect(
    ...
    def capture_image_firebase(self):
        if self.current_frame:
            ...
            self.current_frame = self.current_frame.convertToFormat(
            ...
            frame_rgb = np.frombuffer(
                ptr, dtype=np.uint8).reshape((height, width, 3))
    ...
    def _process_face(self, frame_rgb):
        ...
        if name and student_id:
            ...
            QMessageBox.warning(
                self, "Warning", "Please enter name and student ID.")
    ...
    def start_upload(self, cropped_face, cropped_frame_extended, name, student_id):
        ...
        q_img = QImage(cropped_frame_extended_copy.data, w, h,
                       bytes_per_line, QImage.Format_RGB888)
        pixmap = QPixmap.fromImage(q_img)
        ...
        self.ui.progressBar.setValue(0)
        self.firebase_upload_thread.set_data(
            cropped_face, cropped_frame_extended_copy, name, student_id)
